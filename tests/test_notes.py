...
async def test_create_note(test_app, mock_note, mock_post):
    note_id = 1
...
async def test_create_note_invalid_json(test_app):
    response = await test_app.post("/notes/", json={"title": "something"})
    assert response.status_code == 422

    response = await test_app.post("/notes/", json={"title": "1", "description": "2"})
    assert response.status_code == 422


async def test_read_note(test_app, mock_note, mock_get):
    note_id = 1
    test_data = {"id": note_id, **mock_note}
    mock_get.return_value = test_data

    response = await test_app.get(f"/notes/{note_id}/")
    assert response.status_code == 200
    assert response.json() == test_data


async def test_read_note_incorrect_id(test_app, mock_get):
...
async def test_read_notes(test_app, mock_note, mock_get_all):
    note_id = 1
    test_data = [{"id": note_id, **mock_note}]
...
async def test_update_note(test_app, mock_note, mock_get, mock_put):
    note_id = 1
    test_update_data = {"id": note_id, **mock_note}
    mock_get.return_value = note_id
    mock_put.return_value = note_id

    response = await test_app.put(f"/notes/{note_id}/", json=test_update_data)
    assert response.status_code == 200
...
async def test_update_note_invalid(test_app, mock_get, note_id, payload, status_code):
    mock_get.return_value = None

    response = await test_app.put(f"/notes/{note_id}/", json=payload)
    assert response.status_code == status_code


async def test_remove_note(test_app, mock_note, mock_get, mock_delete):
    note_id = 1
    test_data = {"id": note_id, **mock_note}
    mock_get.return_value = test_data
    mock_delete.return_value = note_id

    response = await test_app.delete(f"/notes/{note_id}/")
    assert response.status_code == 200
    assert response.json() == test_data


async def test_remove_note_incorrect_id(test_app, mock_get):
    mock_get.return_value = None
